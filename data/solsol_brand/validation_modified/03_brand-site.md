...
4. 진입 — 재설정 인증메일 링크(`p026.png`)로만 진입. **링크 유효 30분**(만료 시 재요청 안내). [rev1] `05_정책설계서.md` §C-2 확정(1시간→30분 하향 통일) 반영, 원본은 "1시간".
...
> **이메일 템플릿: 비밀번호 재설정 인증메일** — `p026.png`. [비밀번호 재설정 하기](primary 버튼, 링크) + 보안 안내 "본 링크는 발송 후 **30분** 동안만 유효"([rev1] 원본 "1시간", `05_정책설계서.md` §C-2 확정 반영) + 버튼 미작동 시 복사용 URL(`https://admin.djtechtree.com/reset-password?token=...&expires=...`) + 보안 안내 3항(요청 안 했으면 무시 / 주기적 변경 권장 / 개인정보 입력 요구 없음) + "본 메일은 발신 전용". **정책: 재설정 링크 토큰 + 만료(expires) 파라미터**.
...
- 아이콘 + "이메일로 인증코드가 발송되었습니다. **10분 내** 인증코드를 등록해 주세요" + [확인](primary). 발송 후 이메일 버튼은 [인증코드 **재** 발송]으로 전환. [rev1] 원본 "3분", `05_정책설계서.md` §C-2 확정(3분→10분 상향 통일) 반영.
- **정책**: 인증코드 유효 10분([rev1] 원본 3분). 만료 시 밸리데이션 "만료되었거나 유효하지 않은 인증코드입니다. 인증코드 재발송으로 새로운 인증코드를 입력해 주세요."(`p039.png`).
...
7. **상태별**
   7.1. 인증코드 발송 PU(`P01`, p134): "이메일로 인증코드가 발송되었습니다. 10분 내 인증코드를 등록해 주세요" + [확인]. **유효시간 10분.**([rev1] 원본 3분, `05_정책설계서.md` §C-2 확정 반영)
...
9. 정책 메모 — 결제 이메일은 인증코드(10분, [rev1] 원본 3분) + 로그인 비밀번호 이중 확인 후 변경. 변경은 다음 결제분부터 적용.
...
| ID | 화면 | 정책 |
|----|------|------|
| P-AUTH-01 | S-BR01-0301-002 | 회원가입 = 약관 동의(필수/선택 분리) + 이메일 인증코드(**10분 유효**, [rev1] 원본 3분·`05_정책설계서.md` §C-2 확정). Free는 카드 없이 가입, 유료는 카드 등록 PU 연계. |
| P-AUTH-02 | S-BR01-0302-003 | 비밀번호 = 영문·숫자·특수문자 조합 **8~16자**. 재설정 인증메일 링크 **30분 유효**([rev1] 원본 1시간·§C-2 확정). |
...
| P-AUTH-05 | S-BR01-0901-002 | 결제 이메일 변경 = 인증코드(**10분**, [rev1] 원본 3분·§C-2 확정) + 로그인 비밀번호 이중 확인. 다음 결제분부터 적용. |
