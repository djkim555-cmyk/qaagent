...
> **검산** @spike/br01-r06(2026-07-29, 허브 단일검증·qa 렌즈 단독·실브라우저 라이브): open=3 [br01-r03-D01, br01-r03-D02, br01-r04-D27] (소유트랙: 브랜드) — **해소 0건**. → 종료조건 미충족(**NO-GO**). **D02**는 배포본 실측으로 **미시정 확인**(유료 CTA 3종 전부 `/login?next=/my/sites` 잔존, 기대값 `/login?next=/purchase`). **D01·D27은 재검 불가 `[미확인]`** — `/my/payments`·`/my/inquiry`가 데이터 0건 상태에서 검색창·필터바를 렌더하지 않아(input 0·select 0) 재현 자체가 성립하지 않음 → **선행조건 B-2 시딩이 해소되기 전에는 판정 불가**(라운드를 더 돌려도 `[미확인]` 재생산). 상세·증거 = [`spike/br01-r06.md`](spike/br01-r06.md) · 캡처 [`spike/_r06-evidence/`](spike/_r06-evidence/). ⚠️ **게이트 한계**: 렌즈 독립 스폰 미충족(qa 단독, security·privacy 서명 부재) — 판정이 NO-GO라 잘못된 GO 위험은 없으나 배포 게이트로는 불완전.
>
> **[구] 검산** @storyboard-qa4-20260727(2026-07-27, 총괄 등재·qa 독립발견): open=3 [br01-r03-D01, br01-r03-D02, br01-r04-D27] (소유트랙: 브랜드) — br01-r03-D03 closed 유지, D27 신규 open 추가. → 종료조건 미충족(**NO-GO**, D01·D02·D27 미해소) — 실앱 우회불가. 3건 모두 브랜드 사용자단 세션 소관, 별도 시정 필요.
>
